  ...
  castleTiles.forEach((castleTile) => {
    castleTile.addEventListener("click", () => {
      ...
      if (selectedCastle.amount <= 0) {
        alert(
          "This castle is no longer available. Please pick a valid castle.",
        );
    ...
    tile.addEventListener("click", () => {
      ...
      if (gameState.selected) {
        ...
        board[row][col] = {
          ...gameState.selected,
          castleOwner:
            gameState.selected.amount !== undefined ? gameState.players[gameState.currentPlayerIndex].name : null,
        };
...
export function calculateScores(gameState) {
  ...
  for (let row = 0; row < rows; row++) {
    ...
    segments.forEach((segment) => {
      ...
      const hasDragon = segment.some(
        (cell) => cell && cell.effect === "dragon",
      );
      const hasGoldmine = segment.some(
        (cell) => cell && cell.effect === "goldmine",
      );
